...
\begin{itemize}
    \item \textit{Verification} is the process of proving that a design solution conforms to defined architectural and technical standards. This includes \textit{syntactic verification}, \textit{requirements traceability} and \textit{syntactic verification} \cite{iso15288}.
...
\subsection{Implementing Guidelines in SysML v2} %{ Boelsen, Cibrian }
In contrast to the profile-based approach of SysML v1, SysML v2 implements guidelines through the extension of its underlying metamodel, the \textit{Kernel Modeling Language (KerML)}. Rather than overlaying constraints onto a rigid structure, guidelines in SysML v2 are defined as native language constructs using \textit{Metadata definitions} and semantic specialization. This shift allows for validity to be an intrinsic property of the model elements themselves rather than an externally enforced state.
