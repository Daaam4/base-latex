...
- MSoSA is **2024x Refresh2 HF1 (Academic)**, SysML v2 only behind a feature flag + 3 plugins; licence is VPN-gated; **no Teamwork Cloud** → bridge **Option B** (Java OpenAPI). See `notes/05-msosa-setup.md`.
...
- [ ] Define detailed milestones with Prof. Voss after the exposé (first weekly meeting, end Sep / early Oct)
...
**→ Option B is now the working assumption** (13.09.2026, from the seminar tool material — `notes/05-msosa-setup.md`): the FH has a **desktop/portable MSoSA 2024x Refresh2 HF1 (Academic)** with a floating licence server, and **no Teamwork Cloud is mentioned** — and the standard SysML v2 REST API is served by TWC, not by the desktop client. Option A stays in the thesis as related work / tool-agnosticism argument.

- [ ] **Confirm** no TWC exists (even an eval instance would unlock Option A)
...
### 5.1 Open — ask Prof. Voss / FH IT (first weekly meeting)

**🔴 To ask Prof. Voss (blocking the architecture section of the exposé):**
- [ ] Is a **newer MSoSA build (2025x / 2026x)** available? 2024x SysML v2 is feature-flagged / incubator-grade; later releases are substantially better
- [ ] Is a **Teamwork Cloud** instance available (even eval/trial)? Not mentioned in the course material → assume **no** → bridge Option B
- [ ] Confirm the **statistical design** (§7.2) is what he expects — target is **statistically solid results**, paid for with task breadth; also confirm that evaluating on an **own Anthropic Max subscription** (non-metered, opaque rate limits) is acceptable for the formal work
- [ ] Ask whether he or Höner can act as **second rater** on a ~20% subsample (needed for inter-rater reliability / Cohen's kappa)

**🟡 Resolve myself (technical, not a question for anyone):**
- [ ] Does the MSoSA **Java OpenAPI expose SysML v2 elements**? → §6.2 spike. If not, Option B collapses.

**⏸️ Deferred — answered later, deliberately not blocking now:**
- [ ] Prior/parallel work in Prof. Voss's group not to duplicate (MCP, SysML v2 API, agent evaluation) → **find out after the exposé is submitted**
- [ ] **SysIDE Automator** (Sensmetry, Python API for SysML v2): FH has a licence **only for MSoSA**. Use it only if it turns out to be essential or a significant advantage → **decide at a later stage**. Do not design it into the exposé.
...
- [ ] Export `milestones/Expose.pdf` and send to Prof. Voss **by 20.09**, including the page-limit question and the §5 blockers
