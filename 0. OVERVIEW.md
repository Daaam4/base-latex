...
| D13 | **Bridge = Option B** (MCP ↔ MagicDraw OpenAPI Java plugin in MSoSA). Standing instruction from Prof. Voss: work with what the FH has access to → no TWC, no licence requests. A/A′ = related work / emergency fallback only | 13.09.2026 |
...
Prof. Voss's standing instruction is to **work with what the FH has access to** → no TWC, no licence requests. The bridge is an **MCP server ↔ MagicDraw/Cameo OpenAPI Java plugin inside MSoSA**.
...
~~Teamwork Cloud~~ — **not asked.** Prof. Voss's standing instruction is to **work with what we have access to** → no licence chasing, no TWC. Bridge **Option B** is locked (§4).
...
| 13.09.2026 | **Option B locked** (work with what we have → no TWC); TWC question dropped, only the MSoSA-version question remains |
