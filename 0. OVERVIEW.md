...
- [x] ~~Which **MSoSA version** does the FH have?~~ → **2024x Refresh2 HF1, Academic**, portable zip, OpenJDK 17, SysML v2 behind `-Dmd.sysml2.enabled` + plugins *SysML v2* / *SysML v2 Evaluation* / *SysML v2 Textual Editor*; licence server 149.201.140.104:1101 **via FH VPN**. (Seminar-era — may have been updated.)
- [x] ~~Is the 500-element **Community Edition** cap a problem?~~ → **No**, we have the full Academic licence.
- [ ] Is a **newer build (2025x / 2026x)** available? 2024x SysML v2 is feature-flagged/incubator-grade; later releases are substantially better
- [ ] Is a **Teamwork Cloud** instance available (even eval/trial)? Not mentioned in the course material → assume **no** → bridge Option B
- [ ] Does the MSoSA **Java OpenAPI expose SysML v2 elements**? (If not, Option B collapses — see §6.2 spike)
