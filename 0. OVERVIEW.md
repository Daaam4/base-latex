...
> **This is the only status document.** Appendices A–C (§11) hold the reference detail; source summaries live in `bib/literature/`.
...
- Tooling is fixed: **MSoSA 2024x Refresh2 HF1 (Academic)**, SysML v2 behind a feature flag + 3 plugins, VPN-gated licence, **no TWC** → bridge **Option B** locked (D13). All ~810 runs go through a GUI desktop app → harness must run unattended (§7.2). See §11 (Appendix A).
...
→ The gap between what we have and what exists is **two release generations on exactly the feature we depend on**. This is the highest-leverage question in the whole project: 2026x would likely de-risk the §6.2 spike, the Apollo 11 import, and H2's tool-native arm simultaneously.
...
- [x] **MSoSA version (seminar-era):** **2024x Refresh2 HF1, Academic**, portable zip, OpenJDK 17, SysML v2 behind `-Dmd.sysml2.enabled` + plugins *SysML v2* / *SysML v2 Evaluation* / *SysML v2 Textual Editor*; licence server 149.201.140.104:1101 **via FH VPN** → §11 (Appendix A)
...
- [ ] Install MSoSA per §11 (Appendix A): unpack → add `-Dmd.sysml2.enabled\=d69e2255-b8f5` to `JAVA_ARGS` in `MSOSA/bin/msosa.properties` → OpenJDK 17 → Resource/Plugin Manager → install *SysML v2*, *SysML v2 Evaluation*, *SysML v2 Textual Editor* → restart
...
## 11. Appendix A — MSoSA setup (what the FH has)
...
- The Java OpenAPI was designed for the UML/SysML v1 metamodel → whether it exposes SysML v2 is the **top technical unknown** (§6.2 spike).
- Bridge plugin must target **Java 17**.
- VPN + licence-server dependency → threat to reproducibility, and a failure mode the batch runner must retry on (§7.2).
- **SysML v2 Evaluation** plugin exists → *Verifizierung* and H2's semantic arm have real tool-native capability to expose.

---

## 11b. Appendix B — Model candidates (decision record)
...
## 11c. Appendix C — Deferred design material (pre-kickoff framing)
...
| 13.09.2026 | MSoSA-version question sent to Prof. Voss via Webex; exposé delivery by 20.09 confirmed in the same message. Awaiting answer — does not block literature work |
...
| 13.09.2026 | FH tool version identified: MSoSA 2024x Refresh2 HF1 (Academic), no TWC → bridge Option B becomes working assumption |
