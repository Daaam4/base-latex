...
### 5.1 Ask Prof. Voss **now** — exactly two questions

Nothing else goes to him before the exposé. Both affect the architecture wording, so ask early:

- [ ] **1. MSoSA version** — is a **newer build (2025x / 2026x)** available than the seminar's 2024x Refresh2 HF1? (In 2024x, SysML v2 is feature-flagged / incubator-grade; later releases are substantially better.)
- [ ] **2. Teamwork Cloud** — is a TWC instance available (even eval/trial)? Not mentioned in the course material → assume **no** → bridge **Option B**. A yes would unlock Option A and the standard-REST-API framing.
