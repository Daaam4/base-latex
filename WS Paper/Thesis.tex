...
%%% --- RESULTS - SYSML V1 - SEMANTIC EXTENSION VIA STEREOTYPES ---
...
%%% --- RESULTS - SYSML V1 - TOPOLOGICAL ENFORCEMENT VIA OCL ---
...
%%% --- RESULTS - SYSML V2 - METAMODEL-DRIVEN GUIDELINES ---
\subsubsection{Metadata Definition}
In SysML v2, Boelsen et al. demonstrates how to use the \textit{Metadata Definitions} to construct a domain-specific metamodel hierarchies directly within the language architecture by specializing standard KerML constructs \cite{boelsen}.
In his work, the author aimed to implement modeling guidelines for reusable mechanical system elements based on the \textit{motego} methodology. 

%%%% Defining Metamodel (Establishing the rules)
...
// Enforces behavior as constraints, not blocks
...
// Specific component using custom metadata def
#SolutionElementDef def LubMechLineRollingContact {
    // Guideline mandates specific sub-elements:
...
%%% --- RESULTS - SYSML V2 - CONSTRAINTS (Replacing OCL) ---
\subsubsection{Native Constraint Propagation} % {Ratzke}
To demonstrate the efficacy of SysML v2's native constraint handling, we examine the ''Constructive Model Analysis'' approach proposed by Ratzke et al. case study utilizes the formal semantics of KerML to define ranges and logical assertions directly within the model, treating the system definition as a Constraint Satisfaction Problem (CSP) rather than a static diagram \cite{ratzke}.

The study defines a \textit{Tank} part with attributes for width, height, and length. These attributes leverage the use of range-based semantics such as \textit{oneOf} to define permissible dimensions with precise variability.
A requirement usage \textit{tankBigEnough} is nested directly within the part definition, mandating that the derived volume attribute must fall within a specific range  \cite{ratzke}.

In this demonstration, the restriction on volume is propagated backwards through the mathematical definition (width * height * length), automatically reducing the valid domains of the dimensional attributes.
...
// Part definition utilizing native range constraints
part def Tank {
    // 1. Attribute Definition with "oneOf" Semantics
...
// Requirement leveraging native constraint checking
requirement tankBigEnough {
    // 3. Subject Binding
    subject t: Tank;

    // 4. Constraint Assertion
...
\begin{itemize}
    \item \textbf{Source}: Beers
    \item  OCL constraints are embedded within the profile to restrict how stereotypes can be related or attributed
\end{itemize}
...
\begin{itemize}
    \item \textbf{Source}: Kausch
    \item  DLUF avionics case study demonstrates Theorem Proving to verify complex liveness properties that define system behavior over infinite time
\end{itemize}

\begin{itemize}
    \item \textbf{Source}: Molnar
    \item  applied Model Checking to the LightSwitch case study, utilizing tools like Gamma and Imandra to generate concrete execution traces that identify conflicting system states
\end{itemize}
