...
%%% --- RESULTS - SYSML V2 - USAGE DEFINITION SEPARATION ---
...
\subsection{SysML v1}
The native verification capabilities of SysML v1 are inherently limited due to its UML-based architecture. While stereotypes provide a fundamental layer of structural consistency, through restriction of available elements, they offer no intrinsic verification capabilities \cite{beers}.

We discussed how OCL is layered onto profiles to define formal constraints that can be evaluated against the model, thereby enabling basic verification \cite{beers,lu}. However, OCL is declarative and static, which is excellent for checking ''flags'', but to be able to verify that a model conforms to defined technical standards (e.g., Power < 5W), OCL alone is insufficient if the values are derived variables rather than static constants \cite{lu}.

To bridge this gap, external solvers are required to perform mathematical verification by evaluating the constraints defined in OCL against the model's parameters and derived values \cite{lu,kausch}.
However, solvers cannot directly interpret SysML v1 models therefore transformers are required to transform the models into a format compatible with the solver (e.g., OWL, SMT-LIB) \cite{lu,kausch}.

A prominent example of this verification workflow is the approach by Lu et al., based on the Cloud Agility Baseline (CAB) model. In their work, the authors explicitly highlight the semantic limitations of native SysML v1 tools, noting that tools such as Cameo Enterprise Architecture often validate OCL constraints based solely on syntax. Consequently, the native tool reported that the verification was "successful" even when the model contained known inconsistencies.

To perform actual verification, their workflow required mapping the SysML Block Definition Diagrams, State Machine diagrams, and associated OCL invariants into the Web Ontology Language (OWL). This transformation enabled the use of the Pellet OWL reasoner to evaluate the logical consistency of the CAB model. Crucially, the external solver identified that the \textit{Shipment} block was logically equivalent to ''Nothing'' (unsatisfiable) due to conflicting constraints in the state machine, a fatal design flaw that was undetectable within the native SysML v1 environment.

This case study exemplifies the extrinsic nature of SysML v1 verification, where the validity of a design cannot be determined by the modeling environment itself but is contingent upon the successful transformation to an external mathematical domain.

%%% --- Mechanisms for Model Verification - SYSML V2 - CONSTRAINTS (Replacing OCL) ---
\subsection{SysML v2} %{ Ratzke }
\subsubsection{Formal Semantic Constraints} % {Ratzke}
In this section we refer to the work of Ratzke et al. \cite{ratzke} that builds upon the capabilities of SysML v2 native constraints to provide a more generalized constraint evaluation mechanism, especially for early-phase system modeling \cite{ratzke}.
Ratzke's work introduces range-based semantics that enable modelers to enforce guidelines regarding system variability and precision throughout three constraints:
\begin{itemize}
...
To demonstrate the practical application of these constraints, a \textit{Tank} part definition was modeled with attributes for width, height, and length (see Listing \ref{lst:tank_example}). These attributes leverage the \textit{oneOf} operator, to define permissible dimensions with precise variability. 
The requirement \textit{tankBigEnough} then enforces that the derived volume attribute falls within a specific range.
...
When a restriction is imposed on the final volume, the solver utilizes constraint propagation to automatically reduce the valid domains of the dimensional attributes ($width \times height \times length$), ensuring the system model remains consistent with the specified design guidelines.

%
% Discuss & Implications
%
\section{Discussion and Implications}
\subsection{Stereotypes vs. Metadata}
\begin{itemize}
    \item Stereotypes - Compliance is extrinsic. the model structure and the rule are separate entities (Profiles + OCL) \cite{beers}.
    \item Metadata - Compliance is intrinsic. the rule is the definition of the element itself \cite{boelsen}.
\end{itemize}

\subsection{The Impact on Verification Capabilities}
\begin{itemize}
    \item SysML v1 - They ensure the diagram is readable and topologically correct, but they do not ensure the math is sound \cite{cibrian}.
    \item Metadata - Because the guidelines enforce structure using KerML (First-Order Logic), the resulting models are natively compatible with formal solvers \cite{kausch,molnar}
\end{itemize}

\subsection{Scalability via Library First Verification}
\begin{itemize}
    \item SysML v1 - Complex OCL constraints often have to be re-evaluated for every instance in the system, leading to performance bottlenecks in large models. \cite{bergemann,ratzke} 
    \item SysML v2 - Separation of Definition (Type) and Usage (Instance) allows for a ''Verify Once, Use Many'' approach \cite{boelsen}
\end{itemize}


\subsection{Solver Agnosticism and the Shift from Compliance Checking to Constructive Analysis}

% SysML v1 required a "trust but verify" approach, where users were trusted to apply stereotypes correctly, and scripts verified compliance later. SysML v2 sought to invert this paradigm. By integrating the definition of metadata and constraints into the core language kernel, SysML v2 aims to make non-compliant modeling physically impossible—or at least syntactically invalid—within the tool environment. The shift is from "compliance upon inspection" to "compliance by definition". \cite{cibrian}
