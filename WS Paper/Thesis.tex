...
% --- Mechanisms for Model Verification - SYSML V1 ---
...
\subsection{SysML v2: Formal Verification via Constraint Satisfaction} %{ Ratzke }
In contrast to SysML v1, where verification requires model transformations to external formalisms (e.g., OWL) to check logical consistency, SysML v2 native constraints are built upon KerML, which is founded on first-order logic. This foundation allows verification guidelines to be treated as intrinsic mathematical expressions that can be solved directly within the system model context.

A primary verification workflows is based on reformulating the models into constraint satisfaction problems (CSPs) that can be solved using specialized solvers \cite{ratzke}. This process begins with the compilation of the model into a KerML abstract representation. To render the model solvable, the abstract syntax is ''flattened'' to represent specific instances rather than generic types. This involves cloning features from general definitions to specialized usages and utilizing KerML binding relationships to identify set-intersections between variables and their constraints. The result is a mathematically rigorous CSP consisting of a finite set of variables and their respective domains (Boolean, Integer, or Real), which can then be processed by specialized solvers to enforce logical consistency.

Ratzke et al. [15] demonstrate a concrete application of this workflow by introducing range-based semantics that enable the verification of system variability and precision. In their implementation the semantic library is extended to include three range-based constraint operators:
...
Ratzke et al.'s implementation utilizes a custom solver to apply constraint propagation algorithms. By evaluating the tankBigEnough requirement, this solver actively reduces the design space by automatically restricting the valid domains of the dimensional attributes (width, height, length), ensuring that only values capable of satisfying the volume requirement remain in the valid set.
