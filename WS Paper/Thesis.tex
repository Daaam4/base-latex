...
\lstdefinelanguage{SysMLv2}{
  keywords={package, import, metadata, def, part, constraint, attribute, doc, ref, alias, bind, in, out, perform, first, then, done, return},
...
\begin{itemize}
    ...
    \item \textbf{Constraints}: KerML provides the foundation for defining for defining constraints through constructs such as \textit{assert}, \textit{requirement}. These constructs enable the Specification of Boolean conditions that must be true for a model to be valid \cite{ratzke}.
...
\subsubsection{Contraints} % {Ratzke}

\begin{itemize}
    \item \textbf{Source}: Ratzke
    \item Demonstrate how SysML v2's native constraint constructs can replace the need for external OCL constraints to enforce modeling guidelines.
\end{itemize}
