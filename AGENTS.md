...
> **Read by pi and Claude Code** (`CLAUDE.md` is a symlink to this file). **Work only on branch `master-thesis`** — never create or push other branches; `main` is just the bare LaTeX template.
...
| Fixed dates | Exposé **sent** to Prof. Voss (Sep 2026) · **practical work done by 10.12.2026** · submission ≈ 19.01.2027. Further milestones are agreed with Prof. Voss once the weekly group meetings start (not started yet as of 23.09.2026). |
...
| Tooling | FH licence covers **MSoSA only** — **2026x** (access granted by Prof. Voss 13.09.2026; being installed 23.09.2026, native SysML v2), VPN-gated licence server; no Teamwork Cloud known. LLMs: **own Anthropic Max subscription** → Claude family only. |
...
Kickoff with Prof. Voss fixed the thesis scope: a system under test (an open-source SysML v2 model), a benchmark of tasks with ground truth, an MCP bridge to MSoSA as the interface, fault injection for correction-style tasks, and an evaluator outside the agent loop. The research question is in `master/pages/101expose.tex`; there are **no hypotheses** (D19).
...
├── .pi/skills/                ← agent skills (single source), e.g. qualify-paper
...
│   ├── todo - sources/        ← Type 2/3 source summaries + unqualified candidates:
...
- **Notes:** there is no `notes/` directory. Status lives in `master/0. OVERVIEW.md`; reference detail in `.pi/context/`; decisions go in `.pi/context/02-decisions.md` (numbered D-rows) and dead ends / limitations in its dated *Implementation log*; source handling follows the `qualify-paper` skill (Type 1 summaries → `master/bib/summaries/<bibkey>-summary.md`, Type 2/3 stay in `master/todo - sources/`, all registered in `master/todo - sources/0. Index.md`); PDFs go in `master/bib/papers/<bibkey>.pdf`; the literature mind map is `master/bib/mindmap.md`.
- **Working process — write while working:** every practical phase (bridge, benchmark, evaluation) is written up into `master/pages/2xx-*.tex` **as soon as it is done**, while the next phase's implementation continues. A phase is not finished until its prose exists. Page budget: `.pi/context/01-requirements.md`.
...
| 23.09.2026 | Exposé sent; MSoSA 2026x install started. Claude Code onboarded (reads this file via `CLAUDE.md`; skills shared via `.claude/skills/` → `.pi/skills/`); stale context cleaned up; hypotheses formally dropped (D19) |
