...
> The Obsidian vault (`~/Documents/Obsidian Vault/Master Arbeit`) is **deprecated** — do not read from or write to it. Everything lives in this repo under `MA Thesis/`.
...
| Prior work | WS seminar paper (grade 1.3): *Mechanisms for Model Consistency — SysML v1 vs v2 guidelines* (`WS Paper/`), on branch `SysML-modeling-guidelines` |
...
5. Research question + H1–H3 are in `MA Thesis/pages/101expose.tex`.

**Open questions (ask Prof. Voss / FH IT):** MSoSA version (SysML v2 needs 2025x+)? Teamwork Cloud available? LLM API budget? — full list in `MA Thesis/notes/03-kickoff-results.md`.
...
├── WS Paper/, WS Milestones/  ← previous seminar paper (reference only)
└── MA Thesis/                 ← ALL thesis work happens here
...
Later additions (planned): `MA Thesis/models/` (git submodules for Apollo 11 + GfSE), `MA Thesis/pages/2xx-*.tex` (M2 concept / thesis chapters), and a separate code repo for the MCP bridge.

## 4. How to work in this repo

- **Branch:** `master-thesis`. Commit small with prefixes `feat:` / `fix:` / `docs:`. Do not commit build artefacts (see `MA Thesis/.gitignore`); `000report.pdf` and `milestones/*.pdf` are committed on purpose.
- **Build:** `cd "MA Thesis" && make` (latexmk, TeX Live 2025 is installed). Check page count with `pdfinfo 000report.pdf`; extract text with `pdftotext -layout`.
...
2. Skim `MA Thesis/notes/03-kickoff-results.md` (open questions) and the *Next steps* below.
...
- [ ] Add Apollo 11 + GfSE as submodules under `MA Thesis/models/`; parse with the pilot-implementation jar (GfSE `CI/parse.py`) as ground-truth validity check.
