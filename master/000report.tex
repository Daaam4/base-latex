...
% Redefine bibliography name to English
...
\usepackage{xurl} % url + break long URLs anywhere (bibliography howpublished fields)
\Urlmuskip=0mu plus 0mu % no inter-character stretch (avoids letter-spaced URLs in justified text)

\newcommand\tab[1][1cm]{\hspace*{#1}}

\author{Nassim Awabdy}
...
\usepackage{svg}
\usepackage{xcolor} % Better color support
...
\begin{document}
    ...
    \cleardoublepage % First chapter starts on uneven number
    \setstretch{1.25}
    \pagenumbering{arabic} % Move page number to the right

    % M1 Pages
    \include{./pages/101expose}
