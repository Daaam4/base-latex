...
\begin{itemize}
    ...
    \item \textbf{Problem Statement}: Hightlight the shift from V1's profile-based system to V2's formal, metamodel-based approach. \cite{friedenthal, cibrian,zirui}
    ...
    \item \textbf{Thesis}: The change in mechanisms to V2 enables more robust levels of automated verification compared to V1. \cite{molnar,ratzke}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{V2 Implementation - Metamodel-Driven Validation (Consistency by Design)}: V2 implements consistency through a systematic, metamodel-based validation approach. \cite{cibrian}
    \begin{itemize}
    ...
    \item \textbf{Contrasting Abstaction}: Show that V1 required effort when mapping high-level concepts to OCL expressions and transforming models to support external checking, whereas V2 validation operates directly on the formal semantics of the model via its abstract syntax representation. \cite{zavada, cibrian}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Formal Methods (FM) Integration}: foundation of V2 drastically simplifies the application of FM. \cite{molnar, ratzke}
    \begin{itemize}
        \item Verification tools like Gamma, Imandra, SAVVS, and SysMD are being integrated to leverage V2's features (e.g., the standardized API). \cite{molnar, ratzke}
    \end{itemize}
    ...
    \item \textbf{Constraint Satisfaction and Propagation}: The formal constraint capabilities in SysML v2 enable powerful constructive model analysis. 
    This is achieved by reformulating the system model as a Constraint Satisfaction Problem (CSP) and applying Constraint Propagation algorithms to analyze requirements and reduce variable domains. \cite{molnar, ratzke}
